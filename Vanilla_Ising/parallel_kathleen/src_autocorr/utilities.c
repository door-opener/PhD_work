...
void IsingShowNBarr(Latt *l) {
	 ...
	 for (int i=0; i < l->Nsites; i++) {
                ...
                for (int j=0; j < l->nb_cnt; j++) {
                        ...
                        printf("%d,", l->coord_arr[i].nb_arr[j]);
                }
...
void TskMemAlloc_MPI(Task *Tsch, System *s, int Tsteps, int MPI_RANK, int Q) {
        ...
        Tsch->err_E = (double*)calloc(Tsteps, sizeof(double));

        Tsch->M = (double*)calloc(Tsteps, sizeof(double));

        Tsch->X = (double*)calloc(Tsteps, sizeof(double));
        Tsch->err_X = (double*)calloc(Tsteps, sizeof(double));

        Tsch->C = (double*)calloc(Tsteps, sizeof(double));
        Tsch->err_C = (double*)calloc(Tsteps, sizeof(double));

        if (Q == 1) {

                //Number of rows = Tsteps, Number of columns = s->Simtime 
		Tsch->q1_readings = (double**)calloc(Tsteps, sizeof(double*));
		Tsch->q2_readings = (double**)calloc(Tsteps, sizeof(double*));

		for (int i=0; i<Tsteps; i++) {
                
			Tsch->q1_readings[i] = (double*)calloc(s->Simtime, sizeof(double));
			Tsch->q2_readings[i] = (double*)calloc(s->Simtime, sizeof(double));
...
void TskGatherOBS_MPI(Task *Tsch, int Tsteps, double *E, double *err_E, double *M, double *X, double *err_X, double *C, double *err_C) {
        ...
        MPI_Gather(&Tsch->E[0], Tsteps, MPI_DOUBLE, &E[0], Tsteps, MPI_DOUBLE, 0, MPI_COMM_WORLD);

        MPI_Barrier(MPI_COMM_WORLD);

        MPI_Gather(&Tsch->err_E[0], Tsteps, MPI_DOUBLE, &err_E[0], Tsteps, MPI_DOUBLE, 0, MPI_COMM_WORLD);

	MPI_Barrier(MPI_COMM_WORLD);

        MPI_Gather(&Tsch->M[0], Tsteps, MPI_DOUBLE, &M[0], Tsteps, MPI_DOUBLE, 0, MPI_COMM_WORLD);

        MPI_Barrier(MPI_COMM_WORLD);

        MPI_Gather(&Tsch->X[0], Tsteps, MPI_DOUBLE, &X[0], Tsteps, MPI_DOUBLE, 0, MPI_COMM_WORLD);

	MPI_Barrier(MPI_COMM_WORLD);

        MPI_Gather(&Tsch->err_X[0], Tsteps, MPI_DOUBLE, &err_X[0], Tsteps, MPI_DOUBLE, 0, MPI_COMM_WORLD);

        MPI_Barrier(MPI_COMM_WORLD);

        MPI_Gather(&Tsch->C[0], Tsteps, MPI_DOUBLE, &C[0], Tsteps, MPI_DOUBLE, 0, MPI_COMM_WORLD);
        
        MPI_Barrier(MPI_COMM_WORLD);

	MPI_Gather(&Tsch->err_C[0], Tsteps, MPI_DOUBLE, &err_C[0], Tsteps, MPI_DOUBLE, 0, MPI_COMM_WORLD);
