...
def getOuts( in_dir ):
    ...
    return [ ("%s/%s/NiO.out"%(in_dir,x)) for x in shit ]
...
def getData( out_cur ):
    ...
    return ener*toeV, steps, sd, time
...
def main():
    ...
    AFI_outs = getOuts( AFI_src )
    AFII_outs = getOuts( AFII_src )

    AFII_write, FM_write, AFI_write = [], [], []
    EFMAFII_write, EFMAFI_write = [], []

    for i in range(len(AFII_outs)):

        #lc_cur = np.float64(( AFII_outs[i].split("_")[2] ).replace("A",""))
        lc_cur = np.float64( (( AFII_outs[i].split("_")[3] ).replace("A","")).replace("/NiO.out","") )
        #lc_cur = np.float64(( AFII_outs[i].split("_")[5] ).replace("A",""))
        #ang_cur = np.float64( ( AFII_outs[i].split("_")[-1]).replace("ALP/MnO.out",""))

        AFII_E, AFII_st, AFII_sd, AFII_time = getData( AFII_outs[i] )
        AFI_E, AFI_st, AFI_sd, AFI_time = getData( AFI_outs[i] )
        FM_E, FM_st, FM_sd, FM_time = getData( FM_outs[i] )

        AFII_write.append( [lc_cur, AFII_E/2.0, AFII_sd, AFII_st, AFII_time ] )
        AFI_write.append( [lc_cur, AFI_E/2.0, AFI_sd, AFI_st, AFI_time ] )
        FM_write.append( [lc_cur, FM_E/2.0, FM_sd, FM_st, FM_time ] )
        EFM_AFII_cur = (AFII_E - FM_E)
        EFMAFII_write.append( [lc_cur, EFM_AFII_cur] )

    if verbose:

       print("\n-------------------------------------------------------------------\n")
       print(" a0     EAFII (eV)   S/2 (ub)  #SCF   Time(s) ")
       print("\n-------------------------------------------------------------------\n")
       [ print("%4.3f  %16.14f    %6.4f     %2d    %6.4f"%(x[0], x[1], x[2], x[3], x[4])) for x in AFII_write ]

       print("\n-------------------------------------------------------------------\n")
       print(" a0     EAFI (eV)   S/2 (ub)  #SCF   Time(s) ")
       print("\n-------------------------------------------------------------------\n")
       [ print("%4.3f  %16.14f    %6.4f     %2d    %6.4f"%(x[0], x[1], x[2], x[3], x[4])) for x in AFI_write ]

       print("\n-------------------------------------------------------------------\n")
       print(" a0     EFM (eV)   S/2 (ub)  #SCF   Time(s) ")
       print("\n-------------------------------------------------------------------\n")
       [ print("%4.3f  %16.14f    %6.4f     %2d    %6.4f"%(x[0], x[1], x[2], x[3], x[4])) for x in FM_write ]
